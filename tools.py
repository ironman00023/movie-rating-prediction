...
def draw_country_graph(name, data, data_unidirectional):
    graph = torch_geometric.utils.to_networkx(data, to_undirected=False)
    labels_dict = {}
    i = 0
    ...
    edge_type_colors = {
        ...
        ("person", "rev_directed_by", "movie"): "#8B000E",
        ("person", "rev_acted_by", "movie"): "#8B4D9E",
        ("country", "rev_produced_in", "movie"): "#FF4DFF",
    }
    subgraph_nodes = nx.ego_graph(graph, center_index, undirected=True, radius=2)
    subgraph = graph.subgraph(subgraph_nodes)
    pos = {}
    layer_gap = 3.0

    def place_neighbors_in_circle(center_node, neighbors, radius, offset=0):
        angle_step = 2 * np.pi / len(neighbors) if neighbors else 0
        for idx, neighbor in enumerate(neighbors):
            angle = idx * angle_step + offset
            pos[neighbor] = (
                pos[center_node][0] + radius * np.cos(angle),
                pos[center_node][1] + radius * np.sin(angle)
            )

    pos[center_index] = (0, 0)
    center_neighbors = list(graph.neighbors(center_index))
    place_neighbors_in_circle(center_index, center_neighbors, layer_gap)
    for neighbor in center_neighbors:
        neighbors_of_neighbor = list(graph.neighbors(neighbor))
        place_neighbors_in_circle(neighbor, neighbors_of_neighbor, layer_gap / 2, 0.6)
    pos[center_index] = (0, 0)
    ...
    edge_legend_handles = [
        ...
        Line2D([0], [0], color=edge_type_colors[("movie", "produced_in", "country")], lw=2, label='produced in')
    ]
    ...
    edge_weights = [(graph.edges[e].get('weight', 0) + 1) * EDGE_SIZE for e in subgraph.edges()]
...
def draw_graph_with_no_labels(data_unidirectional, name, title=None, predictions_df=None):
    graph = torch_geometric.utils.to_networkx(data_unidirectional, to_undirected=False)
    labels_dict = {}
    i = 0
    movies_count = len(data_unidirectional['movie'].label)
